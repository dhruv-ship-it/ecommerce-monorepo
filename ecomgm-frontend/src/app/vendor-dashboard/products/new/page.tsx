...
export default function AddVendorProduct() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const productsResponse = await fetch("http://localhost:4000/vendor/available-products", {
          headers: {
        ...
        if (productsResponse.ok) {
          ...
          console.log('Available products response data:', productsData);
        ...
        if (couriersResponse.ok) {
          ...
          console.log('Couriers response data:', couriersData);
          setCouriers(Array.isArray(couriersData.couriers) ? couriersData.couriers : []);
        } else if (couriersResponse.status === 401) {
          performAutoLogout("/");
          return;
        } else {
          const errorData = await couriersResponse.json();
          console.error('Error fetching couriers:', errorData);
          setError("Failed to load couriers: " + (errorData.error || "Unknown error"));
        }
      } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:4000/vendor/product/add-existing", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.action === 'stock_increased') {
          alert(`Stock quantity increased successfully! Previous: ${data.previousStock}, Added: ${data.addedStock}, New Total: ${data.newStock}`);
        } else {
          alert(data.message);
        }
        router.push("/vendor-dashboard/products?success=" + encodeURIComponent(
          data.action === 'stock_increased' 
            ? `Stock quantity increased successfully! Previous: ${data.previousStock}, Added: ${data.addedStock}, New Total: ${data.newStock}` 
            : "Product added successfully to your catalog"
        ));
      ...
      setError("Failed to add product. Please check your network connection and try again.");
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <div className="bg-white overflow-hidden shadow rounded-lg">
              <div className="px-4 py-5 sm:p-6">
                <div className="text-center py-12">
                  ...
                  <p className="mt-1 text-sm text-gray-500">All products have already been added to your catalog.</p>
                  <div className="mt-6">
                    <button
                      onClick={() => router.push("/vendor-dashboard/products")}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <select
                    ...
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border placeholder-gray-500"
                  >
                    <option value="" className="text-gray-500">Choose a product</option>
                    {products.map((product) => (
                      <option key={product.ProductId} value={product.ProductId}>
                        {product.Product} (MRP: ₹{product.MRP}) - {product.CategoryName || 'N/A'} / {product.SubCategoryName || 'N/A'}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border placeholder-gray-500"
                  >
                    <option value="" className="text-gray-500">Choose a courier</option>
