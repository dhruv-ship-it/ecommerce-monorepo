  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/su/users?page=${page}&limit=10`,
        {
          headers: {
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4 text-blue-600">
                <span className="text-blue-600 font-bold">Manage Users</span>
              </h3>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {Array.isArray(users) && users.map((user) => (
                      <tr key={user.UserId}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          <Link href={`/su-dashboard/manage-users/edit/${user.UserId}`} className="text-blue-600 hover:text-blue-900">
                            Edit
                          </Link>
                        </td>
                      </tr>
